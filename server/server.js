...
const MAX_CONTEXT_TOKENS = MAX_TOKENS - 1200;
...
app.post('/api/chat', async (req, res) => {
    ...
    // ===== 이미지만 있을 때: Responses API (input_image) =====
    if (imageFiles.length > 0) {
        const userContent = [
            { type: "input_text", text: userPrompt || "" },
            ...imageFiles.map(f => ({
                type: "input_image",
                image_url: `data:${f.type};base64,${f.data}`,
                detail: "auto"
            }))
        ];
        imageFiles.forEach(f => console.log(`🖼️ 이미지 첨부 (Responses): ${f.name}`));
        ...
        try {
            const response = await retryRequest(() =>
                openai.responses.create({
                    ...
                    input: [
                        { role: "system", content: "사용자의 질문에 대한 답변을 해줘" },
                        ...previousMessages.map(({ role, content }) => ({ role, content: typeof content === 'string' ? content : String(content) })),
