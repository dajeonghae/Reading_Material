...
import { trackMessage, trackScroll, trackCtrlF, trackTextSelection, trackResponseTime, trackFocusEvent } from "../../services/trackingService.js";
...
function Chatbot({ showIndex = true }) {
  ...
  const handleScroll = (e) => {
    ...
    const rawPercent = (scrollTop / Math.max(1, scrollHeight - clientHeight)) * 100;
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'f') trackCtrlF();
    };
    const onSelection = () => {
      const sel = window.getSelection()?.toString() || '';
      if (sel.length >= 10) trackTextSelection(sel);
    };
    const onVisibility = () => {
      trackFocusEvent(document.hidden ? 'blur' : 'focus');
    };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setIsWaiting(false);
      trackResponseTime(Math.ceil(userMessage.number / 2), Date.now() - sentAt);
  ...
  const handleCopy = (e) => {
    ...
    messages.forEach((msg, i) => {
      ...
      const label = msg.role === 'user' ? '나의 말' : 'ChatGPT의 말';
  ...
  return (
    <LayoutWrapper>
      <ChatContainer>
        <MessagesContainer ref={scrollContainerRef} onScroll={handleScroll} onCopy={handleCopy}>
          {(() => {
