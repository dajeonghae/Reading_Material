...
import { trackNodeInteraction, trackContextToggle } from "../../services/trackingService";
...
const TooltipNode = ({ data, id }) => {
  ...
  const handleClick = (event) => {
    ...
    trackNodeInteraction(id, data.label);
  ...
  const handleConfirm = () => {
    const action = contextNodeIds.includes(id) ? 'remove' : 'add';
    trackContextToggle(id, data.label, action);
